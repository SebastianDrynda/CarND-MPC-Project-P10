      ...
      if (sdata.size() > 2 && sdata[0] == '4' && sdata[1] == '2') {
        ...
        if (s != "") {
          ...
          if (event == "telemetry") {
            ...
            auto ptsx_trans_waypoints = Eigen::VectorXd(n_waypoints);
            auto ptsy_trans_waypoints = Eigen::VectorXd(n_waypoints);

            for (unsigned int i = 0; i < n_waypoints; i++ ) {
              double d_x = ptsx[i] - px;
              double d_y = ptsy[i] - py;

              ptsx_trans_waypoints( i ) = d_x * cos( -psi ) - d_y * sin( -psi );
              ptsy_trans_waypoints( i ) = d_x * sin( -psi ) + d_y * cos( -psi );
            }

            // Fit polynomial to the points (3rd order)
            auto coeffs = polyfit(ptsx_trans_waypoints, ptsy_trans_waypoints, 3);
